...
$sourcePath = Join-Path $directory "BiBaZu_GUI.urp"
...
while ($beforeChildren.HasChildNodes) {
    [void]$beforeChildren.RemoveChild($beforeChildren.FirstChild)
}
while ($mainChildren.HasChildNodes) {
    [void]$mainChildren.RemoveChild($mainChildren.FirstChild)
}
...
$targetGzip = [IO.Compression.GzipStream]::new(
    $targetStream,
    [IO.Compression.CompressionLevel]::Optimal
)
$targetGzip.Write($outputBytes, 0, $outputBytes.Length)
$targetGzip.Dispose()
