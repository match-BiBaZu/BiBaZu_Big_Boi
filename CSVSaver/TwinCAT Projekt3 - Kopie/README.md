# Device 3 conveyor with existing GUI

Open `TwinCAT Projekt3.sln` in this directory. This is the clean single-motor project;
...
AM8112 serial **00306149**. It uses **CSV mode 9** with no NC axis.
...
use the same ADS symbols as before. The physical motor is now logical motor 1.
Motor 2 has no hardware links and is not required for readiness. Its legacy ADS
symbols remain available, but do not represent another connected motor.
...
supports one or two motors; this MAIN fixes `MotorCount := 1`. All active-motor
...
- Automatic holding-brake control. PLC CoE checks require actual mode 9,
  manual brake release FALSE and torque offset zero before commissioning is enabled.
...
- Input PDOs `1A00`, `1A01`, `1A02`, `1A0C`: position, statusword, actual velocity,
  ...
  participate in the eight motor mappings.
...
download/start, verify `ConveyorDriveSettings.Valid = TRUE`, both conveyor output
commands zero, and Device 3 OP. Normal controls are in the existing GUI.

The initial supervised movement used the existing finite calibration jog:
